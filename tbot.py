...
def start_bot(message):
    ...
    bot.send_message(chat_id, 'Добро пожаловать в автоматизированную систему автопоезда «Россия – моя история»')
    time.sleep(2)
...
def handle_answer(message):
    ...
    if question_index < len(questions):
        expected_answer = questions[question_index]["correct"]

        if message.text == expected_answer and message.text != "Владимир Владимирович Путин":
            user_scores[chat_id] += 1
        elif message.text != expected_answer:
...
token = "7094821440:AAEwNyNq44FNTDztbsMOqJF8GR_D3MXGM9w"  # Убедитесь, что вы вставили свой токен
bot = telebot.TeleBot(token)
...
def handle_query(call):
    ...
    bot.answer_callback_query(call.id)  # Чтобы убрать индикатор загрузки кнопки
...
@bot.callback_query_handler(func=lambda call: call.data in [option for ques in questions for option in ques["options"]])
def handle_answer(call):
    ...
    if question_index < len(questions):
        ...
        if call.data == expected_answer:
            user_scores[chat_id] += 1

        current_question_index[chat_id] += 1
        ask_question(chat_id)  # Запрашиваем следующий вопрос
